...
This isn't working right, not sure what's up yet.

Unless you are using multiple computers or Windows or planning on sharing your 
code with someone using a Windows machine, you should try to use `multicore` option. 
It is leaner on the memory usage by linking to the same address space. Below you 
can see that the memory address space for variables exported to `multisession` are 
...
future_sapply(X = 1:10, function(x) {.Internal(inspect(a))}) == .Internal(inspect(a))
```
