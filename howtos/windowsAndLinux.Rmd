...
2. Search for **Ubuntu**
    * You're looking for the highest number followed by LTS, currently **18.04 LTS**. 
...
2. Then, type `sudo add-apt-repository 'deb https://cloud.r-project.org/bin/linux/ubuntu bionic-cran35/'`
    * `cloud.r-project.org` is the default mirror, however, it is prudent to connect 
    ...
    the command with the Berkeley mirror would look like `sudo add-apt-repository 'deb https://cran.cnr.berkeley.edu/bin/linux/ubuntu bionic-cran35/'`
...
LaTeX is a text-markup language used when generating .Rmd files. Generally, you 
need a text editor to go with your LaTeX installation, but since we'll be accessing 
it through Rstudio, we already have the editor and just the base TeX.  
